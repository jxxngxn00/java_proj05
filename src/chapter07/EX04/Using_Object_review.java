...
class Person{
	// class : 객체를 생성하기 위한 템플릿 (틀)
	// object (객체) <= 인스턴스화
	
	//1. 필드 : 클래스 블락에서 선언된 변수 (Heap)
		// cf) 지역변수 : 메소드 블락에서 선언된 변수 (Stack)
	
	String name;		//이름	: null (기본값)
	float height;		//키		: 0.0 (기본값)
	float weight;		//몸무게	: 0.0 (기본값)
	int age;			//나이	: 0 (기본값)
	
	
	//2. 생성자 (Constructor)
		//1) 리턴타입이 없다. 2) 클래스 이름과 동일 (메소드)
		//필드의 기본값을 초기화
		//생성자 오버로딩 : 생성자 이름은 동일, 식별자 (인풋 매개변수 타입, 갯수)
	
	Person() {	}	//기본생성자 : 인풋 값이 비어있는 생성자, 생략 가능	
...
public class Using_Object_review {
	...
	public static void main(String[] args) {
		...
		System.out.println(jung.getName()+", "+jung.getHeight()+", "
				+jung.getWeight()+", "+jung.getAge());
