...
//클래스 외부에 올 수 있는 것 : 패키지, 임포트(같은 패키지에 존재하지 않는 클래스), 외부클래스

class A {	// 클래스 : 객체를 생성하기 위한 설계도
	// 클래스 내부에 올 수 있는 것(4가지) : 1. 필드(클래스에서 선언된 변수), 2. 생성자, 3. 메소드, 4.이너클래스
	...
	//생성자 : 클래스는 반드시 생성자가 존재해야한다. 특징 : 1. 클래스 이름과 동일, 2. 리턴이 존재하지 않아야 된다.
...
public class Using_Object {
	...
	public static void main(String[] args) {
		...
		//A() : 클래스 내부의 생성자 호출, 기본 생성자 : 입력 매개변수가 없는 생성자, 생략할 수 있다
		...
		// 클래스 : 객체를 생성하기 위한 설계도 
		// 객체 : 클래스를 기반으로 만들어진 인스턴스 (객체) <== 힙 메모리에 로드됨
